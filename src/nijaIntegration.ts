...
import { NWALLET_API_URL, NWALLET_WS_URL, NFTGEN_ORIGIN, SESSION_STORAGE_KEY, LEGACY_SESSION_STORAGE_KEY, NWALLET_SESSION_KEY } from './config/constants';
import { authService } from './services/authService';

// Use the WebSocket URL from constants
const CORRECT_NWALLET_WS_URL = NWALLET_WS_URL;
import { initializeWebSocketConnection } from './utils/activitySync';
import {
  ErrorType,
  ErrorCode,
  createSessionError,
...
// Following the standard: https://eips.ethereum.org/EIPS/eip-1193
...
// Define the NijaWallet provider interface with EIP-1193 compliance
interface NijaWalletProvider {
  ...
  emit?: (event: string, arg: unknown) => void;
  isConnected?: () => boolean;
  // Add ethereum property for nested provider structure
  ethereum?: EthereumProvider;
  // Add support for standard EIP-1193 methods
  connect?: (options?: { [key: string]: unknown }) => Promise<{ chainId: string; accounts: string[] }>;
...
declare global {
  interface Window {
    // Note: We don't redeclare ethereum here to avoid conflicts
    nijaWallet?: NijaWalletProvider;
    nftGenWalletWs?: WebSocket;
    wsConnectionTimeoutId?: ReturnType<typeof setTimeout>;
    nijaWalletConnectionInfo?: {
      sessionId: string;
      address: string;
      timestamp: number;
    };
...
/**
 * Interface for NFT Minting Activity
 */
 ...
 */
export function hasNijaWalletSession(): boolean {
  try {
 ...
 */
export function getNijaWalletSession(): SessionData | null {
  try {
    ...
    try {
      ...
      const sessionData = localStorage.getItem(SESSION_STORAGE_KEY) ||
                          ...
                          localStorage.getItem(LEGACY_SESSION_STORAGE_KEY);
      if (!sessionData) {
        // Check centralized authentication service first
        const authSession = authService.getSession();
        if (authSession && authSession.isValid) {
          console.log('🔐 Using session from centralized auth service');
          const defaultSession: SessionData = {
            address: authSession.address,
            chainId: "0xaa36a7", // Default to Sepolia
            sessionId: authSession.sessionId,
            timestamp: authSession.lastActivity
          };
          return defaultSession;
        }

        // No valid session found - return null to force authentication
        console.warn('⚠️ No valid session found - user needs to authenticate');
        return null;
      ...
      // Check centralized authentication service as fallback
      const authSession = authService.getSession();
      if (authSession && authSession.isValid) {
        console.log('🔐 Using session from centralized auth service (fallback)');
        const defaultSession: SessionData = {
          address: authSession.address,
          chainId: "0xaa36a7", // Default to Sepolia
          sessionId: authSession.sessionId,
    ...
    // Check centralized authentication service as last resort
    const authSession = authService.getSession();
    if (authSession && authSession.isValid) {
      console.log('🔐 Using session from centralized auth service (last resort)');
      const defaultSession: SessionData = {
        address: authSession.address,
        chainId: "0xaa36a7", // Default to Sepolia
        sessionId: authSession.sessionId,
 ...
 */
export async function initializeNijaIntegration(): Promise<boolean> {
  try {
      ...
      try {
        ...
        if (sessionData && sessionData.address) {
          ...
          if (!sessionData.sessionId) {
            sessionData.sessionId = `session_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;
          }
      ...
      try {
        ...
        if (sessionAge > SESSION_EXPIRY) {
          ...
          const sessionError = createSessionError(
            ...
            'Your session has expired. Please reconnect to Nwallet.',
            { sessionAge },
            true,
            'Please reconnect to Nwallet to continue'
          );
        ...
        try {
          ...
          if (!verificationSuccessful) {
            ...
            const sessionError = createSessionError(
              ...
              'Session verification failed. Please reconnect to Nwallet.',
              {},
              true,
              'Please reconnect to Nwallet to continue'
            );
 ...
 */
export async function syncNFTActivity(activity: NFTMintActivity): Promise<boolean> {
  try {
 ...
 */
export function initializeNijaWalletConnection(): void {
 ...
 */
export const getNijaWalletProvider = async (timeoutMs: number = 5000): Promise<EthereumProvider | null> => {
  // Create a promise that resolves with the provider or null
  const providerPromise = new Promise<EthereumProvider | null>((resolve) => {
    ...
    if (typeof window.nijaWallet !== 'undefined') {
      ...
      const nijaWallet = window.nijaWallet as { ethereum?: EthereumProvider };
      ...
      if (typeof window.nijaWallet.request === 'function') {
        ...
        resolve(window.nijaWallet as unknown as EthereumProvider);
        return;
      }
    }
    ...
    try {
      if (window.parent && window.parent !== window) {
        ...
        if (window.parent.ethereum && window.parent.ethereum.isNijaWallet) {
          ...
          resolve(window.parent.ethereum as EthereumProvider);
          return;
        }
        ...
        if (window.parent.nijaWallet) {
          ...
          const parentNijaWallet = window.parent.nijaWallet as { ethereum?: EthereumProvider };
          ...
          if (typeof window.parent.nijaWallet.request === 'function') {
            ...
            resolve(window.parent.nijaWallet as unknown as EthereumProvider);
            return;
          }
        }
      }
  ...
  return Promise.race([providerPromise, timeoutPromise]) as Promise<EthereumProvider | null>;
};
...
export const syncNFTMintActivity = async (activity: NFTMintActivity): Promise<boolean> => {
