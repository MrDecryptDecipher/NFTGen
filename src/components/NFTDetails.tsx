...
import { ArrowTopRightOnSquareIcon as ExternalLinkIcon, ShareIcon, CurrencyDollarIcon as DollarSignIcon, ClockIcon, TagIcon } from '@heroicons/react/24/outline';
import { GlassCard } from './GlassCard';
import { NFTFractionalize } from './NFTFractionalize';
import { useWallet } from '../context/WalletContext';
import { NFTService } from '../services/nftService';
import { toast } from 'react-toastify';
import type { NFT, NFTStatus } from '../types';
...
export const NFTDetails: React.FC = () => {
  ...
  const handleFractionalize = async (config: { supply: number; pricePerFraction: string; minimumPurchase: number }) => {
    if (!isConnected) {
    ...
    try {
      ...
      const nftService = await NFTService.create(provider, signer);
      await nftService.fractionalize(nft.id, config);
      ...
      setNft(prev => {
        ...
        const updatedNFT: NFT = {
          ...prev,
          status: 'FRACTIONALIZED' as NFTStatus,
          fractions: {
            ...prev.fractions,
            supply: parseInt(details.supply),
            remaining: parseInt(details.available),
  ...
  const royaltyPercentage = nft.royalties.percentage;
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-2 gap-6 mt-6">
                <div>
                  ...
                  <p className="text-white mt-1">{nft.status}</p>
                </div>
                <div>
                  <p className="text-white/60 flex items-center gap-2">
                  ...
                  <p className="text-white mt-1">{royaltyPercentage}%</p>
                </div>
              </div>
