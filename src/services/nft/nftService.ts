...
export class NFTService {
  ...
  async mintNFT(options: MintOptions): Promise<{ tokenId: number; tokenURI: string }> {
    ...
    const event = receipt.events?.find((e: ContractEvent) => e.event === 'TokenMinted');
  ...
  async batchMintNFT(
    ...
    const processedMetadata = await Promise.all(
      metadataArray.map(async (metadata, index) => {
        // Upload image to IPFS if provided
    ...
    // Batch mint NFTs
    const tx = await this.contract.batchMintNFT(
      recipient,
      tokenURIs,
      processedMetadata
    );
    ...
    const receipt = await tx.wait();
    const event = receipt.events?.find((e: ContractEvent) => e.event === 'BatchMinted');
  ...
  async getTokenMetadata(tokenId: number): Promise<TokenMetadata> {
    ...
    return {
      ...
      attributes: metadata[4].map((attr: string) => {
  ...
  async updateRoyalty(recipient: string, percentage: number): Promise<void> {
    if (!this.contract) {
      throw new Error('Service not initialized');
    }
  ...
  async getRoyaltyInfo(): Promise<{ recipient: string; percentage: number }> {
    if (!this.contract) {
      throw new Error('Service not initialized');
    }
