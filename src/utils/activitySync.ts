...
let lastHeartbeatResponse: number = 0;
...
function handleNFTActivity(data: Record<string, unknown>) {
  try {
    if (!data.activity) {
    ...
    const mintActivity = {
      type: 'mint' as const, // Use 'as const' to ensure type is exactly 'mint'
      ...
      status: (activityData.status === 'pending' ? 'pending' :
               ...
               activityData.status === 'failed' ? 'failed' : 'pending') as 'pending' | 'success' | 'failed'
    };
...
let activeConnection: { sessionId: string, address: string, timestamp: number } | null = null;

export async function initializeWebSocketConnection(sessionId: string, address: string, wsUrlParam: string = CORRECT_NWALLET_WS_URL): Promise<boolean> {
  ...
  try {
    ...
    try {
      ...
      // Store the Alchemy instance in window for later use
      // Using a properly typed interface extension
      interface WindowWithAlchemy extends Window {
        alchemyInstance?: typeof alchemy;
      }
      (window as WindowWithAlchemy).alchemyInstance = alchemy;
    } catch (alchemyError) {
...
function dispatchWebSocketStatusEvent(status: 'connecting' | 'connected' | 'disconnected' | 'reconnecting' | 'failed' | 'error' | 'unavailable', message: string) {
  if (typeof window !== 'undefined') {
...
export async function syncActivityToNijaWallet(activity: NFTActivity): Promise<boolean> {
  ...
  try {
    ...
    const updatedActivity = {
      ...
      status: enhancedActivity.status === 'pending' ? 'pending' as const :
              enhancedActivity.status === 'success' ? 'success' as const : 'failed' as const,
    ...
    if (ws && ws.readyState === WebSocket.OPEN && session) {
      try {
        ...
        const nijaActivity = {
          type: 'mint' as const, // Force the type to be 'mint' for compatibility
          hash,
          tokenId: uniqueId,
